...
//        SearchRequest searchRequest = new SearchRequest(query, page);
    ...
    @PostMapping("/docs")
    public ResponseEntity<Long> docs(@RequestParam("id") String id,
        ...
        if (id.equals(a)){
            ...
            author = service.findAuthorById(Long.valueOf(id));
        }

        // Delete existing passport file, if it exists
        if (passport != null && !passport.getOriginalFilename().isEmpty()) {
        String existingPassportFileName = author.getPassportPdf();
        ...
        if (diploma != null && !diploma.getOriginalFilename().isEmpty()) {
            String existingDiplomaFileName = author.getDiplomaPdf();
        ...
        if (diplomaScienceRank != null && !diplomaScienceRank.getOriginalFilename().isEmpty()) {
            String existingDiplomaScienceRankFileName = author.getDiplomaScienceRankPdf();
        ...
        if (diplomaScienceDegree != null && !diplomaScienceDegree.getOriginalFilename().isEmpty()) {
            String existingDiplomaScienceDegreeFileName = author.getDiplomaScienceRankPdf();
        ...
        if (noCriminalRecord != null && !noCriminalRecord.getOriginalFilename().isEmpty()) {
            String existingNoCriminalRecordFileName = author.getDiplomaScienceRankPdf();
        ...
        if (healthStatus != null && !healthStatus.getOriginalFilename().isEmpty()) {
            String existingHealthStatusFileName = author.getDiplomaScienceRankPdf();
        ...
        if (employmentBook != null && !employmentBook.getOriginalFilename().isEmpty()) {
            String existingEmploymentBookFileName = author.getDiplomaScienceRankPdf();
        ...
        log.info("/create [author {}]", author);
        service.create(author);
        return ResponseEntity.ok(author.getAuthorId());
    }
}
