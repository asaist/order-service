...
//    Slice<Author> findByFullNameContainingIgnoreCase(@Param("name")String query, Pageable pageable);
    @Query("SELECT a FROM Author a WHERE LOWER(CONCAT(a.lastName, ' ', a.firstName)) LIKE LOWER(concat('%', :name, '%'))")
    Slice<Author> findByLastNameContainsIgnoreCase(@Param("name") String query, Pageable pageable);
    Author findAuthorByPassportPdf(String document);
